...
import {
  WorkExperience,
  Education,
  EmployerProfile as EmployerProfileType,
} from "@/types/employer";
...
const MyProfile = () => {
  ...
  const [uploadProfilePicture, { isLoading: isUploadingPicture }] =
    useUploadProfilePictureMutation();
  const [uploadDemoVideo, { isLoading: isUploadingVideo }] =
    useUploadDemoVideoMutation();
  const [uploadDocument, { isLoading: isUploadingDoc }] =
    useUploadDocumentMutation();
  ...
  if (isError || !profile) {
    return (
      <div className="flex h-screen items-center justify-center text-red-600">
        ...
        <button
          onClick={() => refetch()}
          className="ml-4 px-4 py-2 text-primary border border-primary rounded-lg"
        >
          Retry
        </button>
      </div>
    ...
    <>
      ...
      <div className="min-h-screen bg-page-bg">
        <input
          ...
          onChange={(e) =>
            handleFileUpload(e.target.files?.[0] || null, uploadProfilePicture)
          }
          accept="image/*"
          className="hidden"
        />
        <input
          ...
          onChange={(e) =>
            handleFileUpload(e.target.files?.[0] || null, uploadDemoVideo)
          }
          accept="video/*"
          className="hidden"
        />
        <input
          ...
          onChange={(e) =>
            handleFileUpload(e.target.files?.[0] || null, uploadDocument)
          }
          className="hidden"
        />
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex flex-col md:flex-row items-start md:items-center space-y-4 md:space-y-0 md:space-x-6">
              <div className="relative flex-shrink-0">
                ...
                <button
                  onClick={() => handleFileSelect(profilePictureInputRef)}
                  className="absolute bottom-0 right-0 p-1 bg-white rounded-full shadow-md hover:shadow-lg transition-shadow"
                >
                  {isUploadingPicture ? (
              ...
              <div className="flex-shrink-0">
                <button
                  ...
                  className="px-4 py-2 text-primary border border-primary rounded-lg hover:bg-blue-50 transition-colors font-semibold flex items-center gap-2"
                >
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                    ...
                    <button
                      onClick={() => handleFileSelect(demoVideoInputRef)}
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  >
                    <Plus className="w-4 h-4 mr-2 inline" /> Add Experience
                  </button>
                </div>
                <div className="space-y-4">
                  {profile.workExperience.map((exp) => (
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors"
                  >
                    <Plus className="w-4 h-4 mr-2 inline" /> Add Education
                  </button>
                </div>
                <div className="space-y-4">
                  {profile.education.map((edu) => (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Allow your profile to be discoverable by schools.
                      </p>
                    </div>
                    <button
                      onClick={() => handleVisibilityToggle(!profile.isVisible)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${profile.isVisible ? "bg-primary" : "bg-gray-200"}`}
                    >
                      {isUpdatingVisibility && (
                        <Loader2 className="w-4 h-4 animate-spin text-white" />
                      )}
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${profile.isVisible ? "translate-x-6" : "translate-x-1"}`}
                      />
                    </button>
                  </div>
                </div>
