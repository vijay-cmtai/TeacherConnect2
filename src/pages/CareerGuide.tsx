...
const CategorySection = ({
  ...
  <div className="py-16 sm:py-20 relative overflow-hidden">
    {/* Remove old gradients, use bg-subtle for background */}
    <div
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
      <div className="text-left md:text-center max-w-3xl mx-auto">
        <h2
          className={`text-4xl font-extrabold tracking-tight sm:text-5xl text-main`}
        >
          {title}
        </h2>
...
const CareerGuideHero = ({
  ...
  <section className="bg-main py-16 sm:py-20">
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-main">
      ...
      <form className="mt-8 max-w-xl mx-auto">
        <div className="relative">
          ...
          <Input
            ...
            className="h-14 pl-14 w-full text-md bg-white rounded-full border-2 border-border shadow-lg focus:ring-2 focus:ring-primary/40"
          />
        </div>
      </form>
      <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-3">
        {themes.map((theme) => (
          <button
            ...
            className="group flex items-center justify-center gap-2 bg-white px-4 py-3 rounded-full border border-border shadow-sm hover:shadow-lg hover:border-primary/60 transition-all"
          >
            {theme.icon}
  ...
  const homePageThemes = [
    ...
    },
    {
      ...
      icon: (
        <FileText
          size={20}
          className="text-secondary group-hover:text-emerald-600"
        />
      ),
    },
    {
      ...
      icon: (
        <MessageSquare
          size={20}
          className="text-secondary group-hover:text-sky-600"
        />
      ),
    },
    {
      ...
      icon: (
        <Rocket
          size={20}
          className="text-secondary group-hover:text-indigo-600"
        />
      ),
    },
  ];
  ...
  return (
    <div className="bg-main min-h-screen font-sans flex flex-col">
      ...
      <div className="flex-grow pt-16">
        <div
          className={`sticky z-40 bg-subtle/95 backdrop-blur-sm border-b border-border transition-all duration-300 ${isScrollingUp ? "top-16" : "top-0"}`}
          ref={navRef}
        >
                ...
                {mainNav.map((tab) => (
                  <div key={tab.id} className="relative">
                    <button
                      ...
                      className={`flex items-center text-sm font-semibold px-4 py-5 transition-colors duration-200 ${activeTab === tab.id ? "text-primary" : "text-secondary hover:text-main"}`}
                    >
                      {tab.label}
                      {tab.subTabs && (
                        <ChevronDown
                    ...
                    {tab.subTabs && openDropdown === tab.id && (
                      <div className="absolute left-0 mt-1 w-64 bg-white rounded-lg shadow-xl py-2 z-50 border border-border animate-in fade-in-20 zoom-in-95">
                        {tab.subTabs.map((subTab) => (
                          <a
                            ...
                            className={`block w-full text-left px-4 py-2.5 text-sm transition-colors ${activeSubTab === subTab.id ? "font-semibold text-primary bg-primary/10" : "text-secondary"} hover:bg-main/60`}
                          >
                            {subTab.label}
                          </a>
                        ))}
