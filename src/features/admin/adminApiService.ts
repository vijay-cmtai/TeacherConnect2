...
export const adminApi = apiService.injectEndpoints({
  endpoints: (builder) => ({
    ...
    updateApplicationByAdmin: builder.mutation<
      any,
      { appId: string; body: any }
    >({
      query: ({ appId, body }) => ({
    ...
    forwardInterview: builder.mutation<any, string>({
      query: (appId) => ({
        url: `admin/applications/${appId}/forward-interview`,
        method: "PUT",
      }),
      invalidatesTags: (result, error, appId) => [
        { type: "Application", id: appId },
    ...
    forwardOffer: builder.mutation<any, { appId: string; formData: FormData }>({
      query: ({ appId, formData }) => ({
        url: `admin/applications/${appId}/forward-offer`,
        method: "PUT",
        body: formData,
      }),
      invalidatesTags: (result, error, { appId }) => [
    ...
    updateCollegeProfileByAdmin: builder.mutation<
      any,
      { userId: string; data: any }
    >({
      query: ({ userId, data }) => ({
        url: `admin/users/college/${userId}/profile`,
        method: "PUT",
        body: data,
      }),
      invalidatesTags: (result, error, { userId }) => [
    ...
    updateEmployerProfileByAdmin: builder.mutation<
      any,
      { userId: string; data: any }
    >({
      query: ({ userId, data }) => ({
        url: `admin/users/employer/${userId}/profile`,
        method: "PUT",
        body: data,
      }),
      invalidatesTags: (result, error, { userId }) => [
    ...
    getAllCareerArticles: builder.query<ApiResponse, string | void>({
      query: (category) =>
        category
          ? `/career-articles?category=${encodeURIComponent(category)}`
          : "/career-articles",
      providesTags: (result) =>
        result
          ? [
              ...result.data.map(({ _id }) => ({
                type: "CareerArticle" as const,
                id: _id,
              })),
              { type: "CareerArticle", id: "LIST" },
            ]
          : [{ type: "CareerArticle", id: "LIST" }],
    }),
    getArticleBySlug: builder.query<SingleItemApiResponse, string>({
      ...
      providesTags: (result) =>
        result?.data?._id
          ? [{ type: "CareerArticle", id: result.data._id }]
          : [],
    }),
    ...
    getAllSalaryGuides: builder.query<ApiResponse, void>({
      ...
      providesTags: (result) =>
        result
          ? [
              ...result.data.map(({ _id }) => ({
                type: "SalaryGuide" as const,
                id: _id,
              })),
              { type: "SalaryGuide", id: "LIST" },
            ]
          : [{ type: "SalaryGuide", id: "LIST" }],
    }),
    ...
    getAllPressArticles: builder.query<PressArticle[], void>({
      ...
      providesTags: (result) =>
        result
          ? [
              ...result.map(({ _id }) => ({
                type: "PressArticle" as const,
                id: _id,
              })),
              { type: "PressArticle", id: "LIST" },
            ]
          : [{ type: "PressArticle", id: "LIST" }],
    }),
    getPressArticleById: builder.query<PressArticle, string>({
      query: (id) => `press-articles/${id}`,
      transformResponse: (response: { success: boolean; data: PressArticle }) =>
        response.data,
    ...
    >({
      ...
      invalidatesTags: (result, error, { id }) => [
        { type: "PressArticle", id },
        { type: "PressArticle", id: "LIST" },
      ],
    }),
    deletePressArticle: builder.mutation<
