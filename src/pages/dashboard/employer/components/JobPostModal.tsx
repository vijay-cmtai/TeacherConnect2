...
} from "@/features/api/employerJobApiService";
...
export const JobPostModal = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const mutation = jobToEdit
      ? updateJob({ id: jobToEdit._id, data: finalData })
      : createJob(finalData);

    try {
      await mutation.unwrap();
      toast.success(
        `Job ${jobToEdit ? "updated" : "created"} successfully! It is now pending admin review.`
      );
      onClose();
    } catch (error: any) {
      toast.error(
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {jobToEdit ? "Edit Job Post" : "Create New Job Post"}
          </DialogTitle>
          <DialogDescription>
            Fill in the details below. All job posts are subject to admin review
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-4 pr-2">
          ...
          <div>
            ...
            <Input
              ...
              onChange={handleChange}
              required
              placeholder="e.g., Education, Teaching"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={handleChange}
              required
              placeholder="e.g., Classroom Management, Lesson Planning"
            />
          </div>
          <div>
