...
} from "@/features/api/collegeApplicationsApiService";
import {
  Card,
  CardContent,
  CardHeader,
...
const ScheduleInterviewModal = ({
  ...
  const handleSubmit = async () => {
    if (!details.scheduledOn || !details.interviewType) {
      toast.error("Please provide an interview date and type.");
      return;
    }
    if (details.interviewType === "Online" && !details.meetingLink) {
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent>
        ...
        <div className="space-y-4 py-4">
          <div>
            <Label htmlFor="scheduledOn">Interview Date & Time</Label>
            <Input
              id="scheduledOn"
              type="datetime-local"
              value={details.scheduledOn}
              onChange={(e) =>
                setDetails({ ...details, scheduledOn: e.target.value })
              }
            />
          </div>
          <div>
          ...
          {details.interviewType === "Online" && (
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setDetails({ ...details, meetingLink: e.target.value })
                }
              />
            </div>
          )}
          <div>
            <Label htmlFor="notes">Notes for Admin/Candidate</Label>
            <Textarea
              id="notes"
              placeholder="e.g., Please prepare a 10-minute demo lesson..."
              value={details.notes}
              onChange={(e) =>
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 py-4 overflow-hidden flex-grow">
          <div className="md:col-span-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar border-r">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span>{maskEmail(application.user.email)}</span>
              </div>
              <div className="flex items-center gap-2">
                <Phone size={14} className="text-muted-foreground" />
                <span>{profile.phone ? maskPhone(profile.phone) : "N/A"}</span>
              </div>
            </div>
            <Separator />
                ...
                {profile.documents?.length > 0 ? (
                  profile.documents.map((doc: any, index: number) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-between p-2 bg-muted/50 rounded-md hover:bg-muted"
                    >
...
const InterviewOutcomeButtons = ({
  ...
  onExtendOffer: (appId: string) => void;
}) => {
  ...
  useEffect(() => {
    ...
    const checkTime = () => {
      const decisionTime = new Date(
        new Date(app.interviewDetails.scheduledOn).getTime() + 30 * 60000
      );
      if (new Date() >= decisionTime) {
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently mark the candidate as not selected for this
              role.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
const CollegeShortlist = () => {
  ...
  const getInterviewStatus = (app: any) => {
    ...
    if (app.status === "offer_extended")
      return { label: "Offer Stage", color: "bg-indigo-100 text-indigo-800" };
    if (
    ...
    if (
      ...
      return { label: "Pending Admin", color: "bg-yellow-100 text-yellow-800" };
  ...
  const stats = useMemo(
    () => ({
      ...
      scheduled: candidates.filter((c) => c.status === "interview_scheduled")
        .length,
  ...
  const handleReject = async (appId: string) => {
    try {
      await updateStatus({ appId, status: "rejected" }).unwrap();
  ...
  const handleExtendOffer = (appId: string) =>
    navigate(`/dashboard/college/offer-letter?appId=${appId}`);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage interviews and hiring for top candidates.
        </p>
      </div>
      <Card>
                ...
                <CardContent className="p-4 pt-2 grid grid-cols-1 md:grid-cols-3 gap-4 flex-grow">
                  <div className="md:col-span-2 space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {profile.phone ? maskPhone(profile.phone) : "N/A"}
                      </span>
                    </div>
                    {app.interviewDetails?.scheduledOn && (
                      <div className="bg-muted/50 rounded-lg p-3 mt-2">
                        ...
                        <p className="text-sm">
                          Type: {app.interviewDetails.interviewType}
                        </p>
                  ...
                  <div className="md:col-span-1 flex flex-col justify-center gap-2">
                    {app.status === "shortlisted" && (
                      <Button size="sm" onClick={() => setSchedulingApp(app)}>
      ...
      {viewingProfileApp && (
        <ViewProfileModal
          application={viewingProfileApp}
          onClose={() => setViewingProfileApp(null)}
        />
      )}
      {schedulingApp && (
